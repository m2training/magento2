...
use Wellevate\Practitioner\Model\PractitionerFactory;
use Wellevate\Customer\Helper\Data as WellevateHelper;
...
class Logger
{
     ...
     * @param PractitionerFactory $practitionerFactory
     * @param \Magento\Framework\Stdlib\DateTime\DateTime $date
     * @param WellevateHelper $wellevateHelper
    ...
    public function __construct(
        ...
        PractitionerFactory $practitionerFactory,
        \Magento\Framework\Stdlib\DateTime\DateTime $date,
        WellevateHelper $wellevateHelper,
    ...
    )
    {
        ...
        $this->practitionerFactory = $practitionerFactory;
        $this->wellevateHelper = $wellevateHelper;
        $this->date = $date;
    ...
    )
    {
        ...
        $this->_logPractitionerLatestUpdate($entry);
    ...
    protected function _logPractitionerLatestUpdate($entry)
    {
        $allowedFieldNames = [
        ...
        if (in_array($entry['field_name'], $allowedFieldNames)) {

            $customer = $this->customerFactory->create()->load($entry['magento_customer_id']);
            if ($this->wellevateHelper->isPractitioner($customer)) {
                $practitioner = $this->practitionerFactory->create()->load($customer->getId(), 'customer_id');

                if ($entry['field_name'] == 'Professional Credential') {
                    $practitioner->setLastCredentialUpdate($entry['created_at']);
                    $practitioner->save();
                }

                if ($entry['field_name'] == 'Opted In/Out') {
                    $practitioner->setTaxOptinOptoutUpdate($entry['created_at']);
                    $practitioner->save();
                }

                if ($entry['field_name'] == 'Patient Product Access') {
                    $practitioner->setDispensaryTypeUpdate($entry['created_at']);
                    $practitioner->save();
                }

                if ($entry['field_name'] == 'Patient Access Option') {
                    $practitioner->setPatientAccessOptionsUpdate($entry['created_at']);
                    $practitioner->save();
                }
            }
        }
